...
export function ok<T>(data: T, init?: { status?: number; cookies?: CookieSpec[] }): NextResponse {
  const res = NextResponse.json({ success: true, data }, { status: init?.status ?? 200 });
  for (const c of init?.cookies ?? []) res.cookies.set(c.name, c.value, c.options);
  return res;
}
