...
import { PaymentGateway, PaymentStatus, Prisma, SubscriptionPlan } from '@prisma/client';
...
export class PaymentsService {
  ...
  async getBilling(coachId: string) {
    ...
    return { subscription, plans, payments, simulateMode: !this.bothConfigured() };
  }

  /** Start checkout → returns a redirect URL (real gateway, or dev simulate page). */
  async createCheckout(coachId: string, plan: SubscriptionPlan, gateway: PaymentGateway, locale: string) {
    if (!PLANS[plan]) throw new BadRequestException({ code: 'BAD_PLAN', message: 'Unknown plan' });
    const { amount, currency } = this.amountFor(plan, gateway);

    const payment = await this.prisma.payment.create({
      data: { coachId, plan, gateway, amount, currency, status: PaymentStatus.PENDING },
    });

    const provider = gateway === PaymentGateway.ZARINPAL ? this.zarinpal : this.stripe;
    ...
    if (!provider.configured) {
      ...
      return { redirectUrl: `${appUrl}/${locale}/coach/billing?simulate=${payment.id}` };
    }

    const callbackUrl =
      gateway === PaymentGateway.ZARINPAL
        ? `${this.config.get('BACKEND_PUBLIC_URL')}/api/coach/billing/zarinpal/callback?locale=${locale}`
    ...
    const result = await provider.createCheckout({
      ...
      amount,
      currency,
      description: `FitCoach ${plan} subscription`,
      callbackUrl,
    });
    await this.prisma.payment.update({ where: { id: payment.id }, data: { reference: result.reference } });
  ...
  async devComplete(coachId: string, paymentId: string) {
    ...
    const payment = await this.prisma.payment.findFirst({ where: { id: paymentId, coachId } });
  ...
  async handleZarinpalCallback(authority: string, status: string, locale: string): Promise<string> {
  ...
  async handleStripeWebhook(rawBody: Buffer, signature: string): Promise<void> {
    ...
    if (payment) await this.markPaidAndActivate(payment, event as unknown as Prisma.InputJsonValue);
  }
  ...
  private async markPaidAndActivate(
    ...
    const claimed = await this.prisma.payment.updateMany({
      ...
      data: { status: PaymentStatus.PAID, raw: (raw ?? {}) as Prisma.InputJsonValue },
    });
    if (claimed.count === 0) return; // already processed → no double activation
    const sub = await this.subscriptions.activateOrExtend(payment.coachId, payment.plan);
    await this.prisma.payment.update({ where: { id: payment.id }, data: { subscriptionId: sub.id } });
    this.logger.log(`Payment ${payment.id} paid → subscription ${sub.id} active until ${sub.endsAt.toISOString()}`);
  }
