...
describe("PaymentsService", () => {
  ...
  beforeEach(() => {
    prisma = {
      payment: {
        create: jest
          .fn()
          .mockResolvedValue({ id: "p1", coachId: "c1", plan: "M3" }),
    ...
    subscriptions = {
      activateOrExtend: jest
        .fn()
        .mockResolvedValue({ id: "s1", endsAt: new Date(Date.now() + 1e9) }),
  ...
  it("routes to the in-app simulate flow when no gateway is configured", async () => {
    ...
    expect(res.redirectUrl).toBe(
      "http://localhost:3000/fa/coach/billing?simulate=p1",
    );
  ...
  it("uses the real gateway when configured", async () => {
    ...
    zarinpal.createCheckout.mockResolvedValue({
      redirectUrl: "https://zp/StartPay/AUTH",
      reference: "AUTH",
    });
    const res = await service.createCheckout(
      "c1",
