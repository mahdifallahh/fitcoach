...
import type { CheckoutInput, CheckoutResult, PaymentProvider, VerifyResult } from '../interface';
...
export class ZarinpalProvider implements PaymentProvider {
  ...
  async createCheckout(input: CheckoutInput): Promise<CheckoutResult> {
    ...
    const json = (await res.json()) as { data?: { authority?: string }; errors?: unknown };
    const authority = json?.data?.authority;
    if (!authority) {
      console.error(`[zarinpal] request failed: ${JSON.stringify(json?.errors ?? json)}`);
