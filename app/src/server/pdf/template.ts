...
export function renderProgramHtml(
  ...
  const stats = [
    ...
    program.studentHeightCm != null
      ? `${t.height}: ${program.studentHeightCm}cm`
      : null,
    program.studentWeightKg != null
      ? `${t.weight}: ${program.studentWeightKg}kg`
      : null,
  ]
    .filter(Boolean)
    .join(" · ");
...
<style>
  * { box-sizing: border-box; }
  ...
  .brand { display: flex; align-items: center; gap: 3px; margin-bottom: 10px; direction: ltr; }
  .brand img.mark { height: 24px; width: auto; }
  .brand img.word { height: 13px; width: auto; }
  h1 { font-size: 22px; margin: 0 0 4px; color: ${BRAND.navy}; }
  ...
  .day h2 {
    font-size: 15px; color: ${BRAND.navy}; margin: 0 0 8px;
  ...
  .setsreps { font-weight: 700; color: ${BRAND.blue}; white-space: nowrap; }
  .desc { margin: 4px 0 0; color: #64748b; font-size: 12px; }
  .video { margin: 3px 0 0; font-size: 11px; }
  .video a { color: ${BRAND.blue}; text-decoration: none; }
  ...
  .superset-label { font-size: 11px; font-weight: 700; color: ${BRAND.blue}; margin-bottom: 6px; }
