...
export function CoachesList() {
  ...
  const { data, isLoading } = useStudentCoaches();
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {data.map((coach) => (
            <Link key={coach.coachId} href={`/student/coaches/${coach.coachId}`}>
                ...
                <CardContent className="flex items-center gap-4 p-4">
                  <div className="flex size-12 shrink-0 items-center justify-center overflow-hidden rounded-full bg-muted">
                    {coach.avatarUrl ? (
                      ...
                      <img src={coach.avatarUrl} alt="" className="size-full object-cover" />
                    ) : (
                  ...
                  <Badge variant="secondary">{t('programsCount', { count: coach.programCount })}</Badge>
