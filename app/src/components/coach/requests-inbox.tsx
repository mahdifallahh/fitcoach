...
function RequestCard({ req }: { req: CoachRequest }) {
  ...
  function writeProgram() {
    router.push(
      `/coach/programs/new?student=${encodeURIComponent(req.contact)}&request=${req.id}`,
    );
  }
  ...
  const training =
    req.trainingYears != null || req.trainingMonths != null
      ? t("trainingValue", {
          years: req.trainingYears ?? 0,
          months: req.trainingMonths ?? 0,
        })
      : null;
  const statusVariant =
    req.status === "ACCEPTED"
  ...
  return (
    <Card>
      <CardContent className="space-y-3 p-4">
        ...
        <div className="flex flex-wrap gap-2 text-sm">
          ...
          {req.daysPerWeek != null && (
            <Chip>
              ...
              {t("daysValue", { days: req.daysPerWeek })}
            </Chip>
          )}
        ...
        {req.medicalHistory && (
          <Field label={t("medical")} value={req.medicalHistory} />
        )}
