...
import { useActivateTrial, useBilling, useCheckout, useDevComplete } from '@/lib/query/use-billing';
      ...
      {!sub ? (
        <Card className="max-w-md border-primary/40 bg-primary/5">
          ...
          <CardContent className="space-y-3">
            <p className="text-sm text-muted-foreground">{t('startTrialDesc')}</p>
            <Button onClick={startTrial} disabled={activateTrial.isPending}>
              {activateTrial.isPending ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />}
